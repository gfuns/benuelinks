...
<head>
    ...
    <title>Transfer History (Bulk) | {{ env('APP_NAME') }}</title>
                        ...
                        <table class="data-table dt-filter-init admin-tnx nobreak">
                            <thead>
                                <tr class="data-item data-head">
                                    ...
                                    <th class="data-col sli">File Name</th>
                                    <th class="data-col sli">Beneficiaries</th>
                                    <th class="data-col sli">Total Amount (&#8358;)</th>
                                    <th class="data-col sli">Date</th>
                                    <th class="data-col sli">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($files as $data)
                                    <tr class="data-item" id="tnx-item-{{ $data->id }}">
                                        ...
                                        <td class="data-col dt-token">
                                            <span class="sub sub-s2 token-amount">{{ $data->file_name }}</span>
                                        </td>

                                        <td class="data-col dt-token">
                                            <span class="sub sub-s2 token-amount">
                                                {{ $data->beneficiaries->count() }}</span>
                                        </td>

                                        <td class="data-col dt-token">
                                            <span class="sub sub-s2 token-amount">&#8358;{{ number_format($data->beneficiaries->sum("amount"), 2) }}</span>
                                        </td>

                                       <td class="data-col dt-token">
                                            <span
                                                class="sub sub-s2 token-amount">{{ date_format($data->created_at, 'jS M, Y g:i:s a') }}</span>
                                        </td>
                                        <td class="data-col dt-token">
                                            <span class="sub sub-s2 amount-pay">
                                                <a href="{{ route("bulkTransferBeneficiaries", [$data->id]) }}">
                                                    <label class="btn-primary btn-xs" type="button"
                                                        style="font-weight:bold">View Beneficiaries</label></a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
